...
let
  ...
  llamaccpPackage = llama-cpp.override {
    ...
    cmake = if enableCuda then cudaCmake else cmake;
    stdenv = if enableCuda then gcc11Stdenv else stdenv;
  } // optionals cudaSupport { inherit cudaPackages; };
  ...
  cudaCmake = cmake.override { stdenv = gcc11Stdenv; };
  cuda_cudart = cudaPackages.cuda_cudart.override { stdenv = gcc11Stdenv; backendStdenv = gcc11Stdenv; };
  cudatoolkit_gcc11 = cudatoolkit.override { backendStdenv = gcc11Stdenv; };
...
rustPlatform.buildRustPackage {
  ...
  cargoBuildFlags =
    ...
    buildFeatures = [ "ee" ]
    ++ optionals enableCuda [ "cuda" ]
    ++ optionals enableRocm [ "rocm" ];
  ...
  shellHook = if cudaSupport then ''
    export LD_LIBRARY_PATH = "${cudatoolkit_gcc11}/lib:${cudatoolkit_gcc11}/include";
    export CUDA_PATH = "${cudatoolkit_gcc11}";
    export EXTRA_LDFLAGS = "-L/lib -L${cudatoolkit_gcc11}/lib -L${cudatoolkit_gcc11}/include";
